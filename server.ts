...
function getGenAi(): GoogleGenAI {
  ...
  if (!apiKey) {
    throw new Error("GEMINI_API_KEY environment variable is not configured. Please add it to your environment or Settings > Secrets.");
  }
  if (!genAiClient) {
...
app.post("/api/tts/generate", async (req, res) => {
  ...
  try {
    ...
    // Construct prompt adhering strictly to prompt construction requirements:
    // Combining:
    // A. Voice Profile settings
    // B. Voice Direction
    // C. Scene
    // D. Sample Context
    // E. Text (Authoritative spoken transcript)
    const promptParts: string[] = [];

    promptParts.push("Perform text-to-speech narration with the following vocal parameters and performance instructions:");
    
    // Vocal identity details
    ...
    console.log(`[TTS] Generating narration: voice=${voice}, chars=${trimmedText.length}, model=${model}`);

    // Call official Gemini TTS API with speechConfig
    ...
    // Look for audio part with inlineData
    ...
    if (!audioBase64) {
      console.warn("[TTS] No inline audio data found in response parts. Checking candidates text/finishReason:", candidate?.finishReason);
      return res.status(502).json({
        error: "Generation Failed",
        message: "The model completed the request without producing audio data. This may be due to content moderation or temporary API unavailability. Please try again with slight adjustments."
      });
    }

    const latencyMs = Date.now() - startTime;
    console.log(`[TTS] Success: generated in ${latencyMs}ms, bytes=${audioBase64.length}`);
    ...
    const latencyMs = Date.now() - startTime;
    console.error(`[TTS Error] Failed after ${latencyMs}ms:`, err);
    ...
    if (errorMessage.includes("API_KEY") || errorMessage.includes("unauthorized") || errorMessage.includes("PERMISSION_DENIED") || errorMessage.includes("API key")) {
      statusCode = 401;
      userFriendly = "Gemini API key is invalid or not configured. Ensure GEMINI_API_KEY is active in Settings > Secrets.";
